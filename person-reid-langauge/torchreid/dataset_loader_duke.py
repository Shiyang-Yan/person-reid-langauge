...
def read_sentence(img_path):
    imgs = img_path.split('/')
    all_captions = json.load(open('/media/yanshiyang/DATA3/deep-person-reid/data/dukemtmc-reid/out.json', 'r'))  
    key_image = 'duke_'+ imgs[-1]
    #print(key_image)
    sentence = all_captions[key_image]
    sentence = sentence.replace('caption:', '')
    list_sent = sentence.strip().split()
    worddict_tmp = pickle.load(open('/media/yanshiyang/DATA3/deep-person-reid/data//wordlist_reid.p', 'rb'))
